...
class TechniciansFragment : Fragment() {
    ...
    private fun showAddOrEditTechnicianDialog(technician: Technician?) {
        ...
        if (isEditMode) {
            nameEditText.setText(technician?.nombre)
            addressEditText.setText(technician?.direccion)
            phoneEditText.setText(technician?.telefono)
            emailEditText.setText(technician?.correo)
